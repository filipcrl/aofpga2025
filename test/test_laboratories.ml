...
module Dut_4b = Laboratories.Make (struct
  let word_w = 4
  let line_w = 15
  let acc_w = 8
end)
...
let words_of_line ~word_w line =
  ...
  let padded_w = padded_line_w ~line_w:(width bits) ~word_w in
...
let simulate_and_capture
  ...
  =
  let words =
    List.concat_map lines ~f:(fun line -> words_of_line ~word_w line)
  in
...
let run_input
  ...
  ~gap
  =
  let lines = strip_and_filter_lines input in
  let line_w = String.length (List.hd_exn lines) in
  let beams_bits, result =
    simulate_and_capture ?save_path (module Dut) ~word_w ~lines ~gap
  in
  let beams =
    Bits.split_msb ~part_width:(padded_line_w ~line_w ~word_w) beams_bits
  in
...
let run_bigtest
  ...
  let _beams_bits, result =
    simulate_and_capture (module Dut) ~word_w ~lines ~gap
  in
  printf "result=%d\n" result

let%expect_test "short input, 1b word, gap 0" =
  run_input (module Short_dut_1b) ~input:short_input ~word_w:1 ~gap:(Fixed 0);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 1b word, gap 1" =
  run_input (module Short_dut_1b) ~input:short_input ~word_w:1 ~gap:(Fixed 1);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 1b word, gap 2" =
  run_input (module Short_dut_1b) ~input:short_input ~word_w:1 ~gap:(Fixed 2);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 1b word, seeded random gap" =
  run_input (module Short_dut_1b) ~input:short_input ~word_w:1 ~gap:(Random { seed = 123; max_gap = 2 });
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 2b word, gap 0" =
  run_input (module Short_dut_2b) ~input:short_input ~word_w:2 ~gap:(Fixed 0);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 2b word, gap 1" =
  run_input (module Short_dut_2b) ~input:short_input ~word_w:2 ~gap:(Fixed 1);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 2b word, gap 2" =
  run_input (module Short_dut_2b) ~input:short_input ~word_w:2 ~gap:(Fixed 2);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 2b word, seeded random gap" =
  run_input (module Short_dut_2b) ~input:short_input ~word_w:2 ~gap:(Random { seed = 123; max_gap = 2 });
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 3b word, gap 0" =
  run_input (module Short_dut_3b) ~input:short_input ~word_w:3 ~gap:(Fixed 0);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 3b word, gap 1" =
  run_input (module Short_dut_3b) ~input:short_input ~word_w:3 ~gap:(Fixed 1);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 3b word, gap 2" =
  run_input (module Short_dut_3b) ~input:short_input ~word_w:3 ~gap:(Fixed 2);
  [%expect {|
    ..S..
    .|^|.
    |^||.
...
let%expect_test "short input, 3b word, seeded random gap" =
  run_input (module Short_dut_3b) ~input:short_input ~word_w:3 ~gap:(Random { seed = 123; max_gap = 2 });
  [%expect {|
    ..S..
    .|^|.
    |^||.
